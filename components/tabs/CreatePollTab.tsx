...
export const CreatePollTab = ({ fid }: CreatePollTabProps) => {
  ...
  useEffect(() => {
    const fetchPolls = async () => {
      try {
        ...
        const pollPromises = mine.map(async (ca: string) => {
  ...
  return (
    <div className="p-6 bg-white rounded-2xl transition-all durationwebkit-transition-all">
      ...
      {loading ? (
        ...
        <p className="mt-4 text-gray-600">No polls created yet.</p>
      ) : (
